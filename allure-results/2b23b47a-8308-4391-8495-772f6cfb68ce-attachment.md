...
Call log:
  - waiting for getByText('Sauce Labs Backpack', { exact: true }).locator(getByRole('button', { name: 'Add to cart' }))
...
```ts
  ...
  2  |     constructor(page) {
  3  |         this.page = page;
  4  |         this.productItem = page.getByText('Sauce Labs Backpack', { exact: true });
